...
dim = 3;
N0 = 10; % number of initial design points (N0^dim)

% design space bounds
a1 = -1; b1 = 1;

A = [a1 a1 a1]; % array of bounds for get_neighbour function
B =[b1 b1 b1];
...
q = 14; % number of parameters
theta = ones(q,1);

% Compute components of FIM
I0 = zeros(q,q,N0^dim);
for i = 1:N0^dim
    z = [1 ; x(i,1); x(i,2); x(i,3); x(i,1)*x(i,2); x(i,1)*x(i,3); x(i,2)*x(i,3); x(i,1)^2; x(i,2)^2; x(i,3)^2; x(i,1)*x(i,2)*x(i,3); x(i,1)^3; x(i,2)^3; x(i,3)^3];
    I0(:,:,i) = (exp(z'*theta)/(1+exp(z'*theta))^2)*(z*z');
end
